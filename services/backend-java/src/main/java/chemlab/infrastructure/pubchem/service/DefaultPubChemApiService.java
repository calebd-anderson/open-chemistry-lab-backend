...
public class DefaultPubChemApiService implements PubChemApiService {
    ...
    @Override
    public FastformulaCidsResponse getFormulaCids(String formula) throws PugApiException {
        log.trace("Calling PubChem API with molecular formula: {}", formula);
        String pubChemUrl = PUG_PROLOG + PUG_INPUT + formula + PUG_CIDS_OPERATION + PUG_OUTPUT;
        try {
            log.trace("Sending PugAPI url in service: {}", pubChemUrl);
            FastformulaCidsResponse pugApiValue = restTemplate.getForObject(pubChemUrl, FastformulaCidsResponse.class);
            assert pugApiValue != null;
            return pugApiValue;
        } catch (HttpStatusCodeException exception) {
            log.warn("Received {} error response from PUG API: {}", exception.getStatusCode(), exception.getResponseBodyAsString());
